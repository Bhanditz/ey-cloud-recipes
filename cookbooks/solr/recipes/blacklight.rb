  ...
  node[:applications].each do |app_name, data|
    ...
    solr_hostname = if node[:solr_utility_name].empty?
      ...
      if solr_utility = node[:utility_instances].find { |utility| utility[:name] == node[:solr_utility_name] }
        ...
        Chef::Log.info "Blacklight was not configured because no there is no utility instance named #{node[:solr_utility_name]}"
        nil
      end
    end
    
    unless solr_hostname.nil?
      template "/data/#{app_name}/shared/config/solr.yml" do
        source "solr.yml.erb"
